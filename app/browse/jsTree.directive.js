    ...
    managePlugins: function(s, e, a, config) {
      if (a.treePlugins) {
        ...
        if (config.plugins.indexOf('search') >= 0) {
          ...
          if (a.treeSearchInput) {
            searchbox = jQuery('#' + a.treeSearchInput);
          };
          if (!searchbox) {
            searchbox = e.next();
            if (searchbox.attr('class') !== 'ng-tree-search') {
              searchbox = e.after('<input type="text" placeholder="Search Tree" class="ng-tree-search"/>')
                .next();
            }
          }
          searchbox
        ...
        if (config.plugins.indexOf('massload') >= 0 ) {
          ...
          if (a.treeMassloadUrl) {
            config.massload.url = a.treeMassloadUrl;
          };
        }
